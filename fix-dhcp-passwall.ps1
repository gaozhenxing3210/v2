...
parser = argparse.ArgumentParser(description="Fix OpenWrt/Passwall DHCP assignment issue.")
...
echo "[2/5] Excluding DHCP UDP ports from Passwall..."
...
echo "[3/5] Making the fix persistent in Passwall startup normalizer..."
if [ -f /usr/share/passwall/acl_normalize.sh ]; then
  ...
  grep -q "udp_no_redir_ports '67,68'" /usr/share/passwall/acl_normalize.sh || true
fi

uci -q commit dhcp
uci -q commit passwall

echo "[4/5] Restarting DHCP/DNS and Passwall..."
...
client.connect(args.host, username=args.user, password=args.password, timeout=12, look_for_keys=False, allow_agent=False)
...
try {
  python $tmp --host $HostIp --user $User --password $Password
}
finally {
